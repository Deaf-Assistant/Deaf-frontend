...
export default function Header() {
  ...
  // เพิ่ม state isAdmin เพื่อความชัวร์ในการแสดงผล
  const [isAdmin, setIsAdmin] = useState(false);

  // ฟังก์ชันเช็คข้อมูล User และ Role
  const checkUser = () => {
  ...
  useEffect(() => {
    ...
    // 2. เช็คทุกครั้งที่เปลี่ยนหน้า (Pathname เปลี่ยน)
    checkUser();

    // 3. เพิ่ม Event Listener ดักการเปลี่ยนแปลง (Login/Logout/Storage Change)
    window.addEventListener('storage', checkUser); // ดักการแก้ LocalStorage จาก Tab อื่น
    window.addEventListener('auth-change', checkUser); // ดัก Event ที่เราอาจจะสร้างขึ้นเองตอน Login
    window.addEventListener('focus', checkUser); // ดักตอนสลับหน้าจอ (ช่วยตอนแก้ Role ใน LocalStorage แล้วกลับมาดู)
  ...
  }, [pathname]); // run เมื่อ pathname เปลี่ยนด้วย

  const handleLogout = () => {
    auth.logout();
    checkUser(); // อัปเดต state ทันที
      ...
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between h-20">
          ...
          <nav className="hidden md:flex items-center space-x-1">
            ...
            {/* ใช้ state isAdmin แทนการเรียก auth.isAdmin() โดยตรง เพื่อให้ Reactive */}
            {user && isAdmin && (
              <Link
