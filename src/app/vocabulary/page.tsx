...
export default function VocabularyPage() {
  ...
  const loadData = async () => {
    try {
      ...
      // alert('ไม่สามารถโหลดข้อมูลได้'); // ปิด alert เพื่อ UX
    } finally {
      setLoading(false);
    }
  };
  ...
  const performSearch = async (keyword: string, vocabList = vocabularies) => {
    ...
    try {
      // 1. ลองค้นหาจาก API (Server-side search)
      ...
      // 2. ถ้า API พัง ให้ค้นหาแบบ Client-side (แก้ชื่อตัวแปรให้ตรง DB)
  ...
  const filterVocabularies = () => {
    ...
    // Filter by course (แก้เป็น course_id)
    ...
    if (searchKeyword.trim()) {
      // ใช้ Client-side filter เบื้องต้นเพื่อความเร็ว (แก้ชื่อตัวแปรแล้ว)
