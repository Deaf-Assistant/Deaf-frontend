...
export async function GET(req: NextRequest) {
    try {
        ...
        if (tokenError || !callerUser) return NextResponse.json({ ok: false, message: "Invalid token" }, { status: 401 });

        const { data: callerProfile } = await supabaseAdmin.from("users").select("role").eq("id", callerUser.id).single();
        if (callerProfile?.role !== "ADMIN") return NextResponse.json({ ok: false, message: "Permission denied" }, { status: 403 });
        ...
        const { data: vocabs } = await supabaseAdmin
            .from("vocabularies")
            .select("id, term_thai, image_url, image_url2, image_url3, video_url, fingerspelling_video_url");
        ...
        (vocabs ?? []).forEach((v: any) => {
            [v.image_url, v.image_url2, v.image_url3, v.video_url, v.fingerspelling_video_url].forEach((url: string | null) => {
        ...
        const buildFileEntry = (file: any, bucket: string, baseUrl: string) => {
            ...
            if (!isOrphaned) {
                const match = (vocabs ?? []).find((v: any) =>
                    [v.image_url, v.image_url2, v.image_url3, v.video_url, v.fingerspelling_video_url].includes(publicUrl)
                );
        ...
        const imageEntries = imageFiles.map((f) => buildFileEntry(f, "images", imageBase));
        const videoEntries = videoFiles.map((f) => buildFileEntry(f, "videos", videoBase));
...
export async function DELETE(req: NextRequest) {
    try {
        ...
        const { data: callerProfile } = await supabaseAdmin.from("users").select("id, name, role").eq("id", callerUser.id).single();
        if (callerProfile?.role !== "ADMIN") return NextResponse.json({ ok: false, message: "Permission denied" }, { status: 403 });

        const { bucket, name } = await req.json();
        if (!bucket || !name) return NextResponse.json({ ok: false, message: "bucket and name are required" }, { status: 400 });
